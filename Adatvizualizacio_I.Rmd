...
# Forrás lista beolvasása
biblio <- ReadZotero(user = "uid", .params = list(collection = 'cid', key="key"),delete.file = TRUE)

# Forrásokhoz tartozó kulcsok listázása a hivatkozásokhoz
# Futtatáshoz közvetlenül nem kell, csak köztes munkához
refs <- biblio %>% {
  .$title
} %>%
  bind_rows() %>%
...
Az óra anyagai elsõsorban az R for Data Science könyvre épülnek `r Cite(biblio['wickham_r_2017'])`
...
ggplot(data = gapminder_continent_pop_summary, aes(x = year, y=continent_total_pop_m, group=continent)) + 
  ...
  # A kitöltéshez tartozó jelmagyarázat a hozzá tartozó színskálán belül módosítható
  scale_fill_manual(values = c("#ffa500", "#83b100","#2b8a00","#878c8e","#add8e6"),
                    name = "Kontinens",
...
Emellett kisebb munkát igényel, és érdemes lehet a gyakran használt segédfüggvényeket, mint például a formázáshoz használt `num_m2b_2_digits()` függvény, egy külön scriptben elmenteni, ami aztán az új scriptek elején betölthetõ a `source()` függvénnyel, és felhasználható az új chartokhoz.

## Házi feladat

1. A plot függvény segítségével állítsatok elõ vonaldiagramot a várható élettartam alakulásáról Ausztriában 1990 után. A vonal színe legyen az alapbeállítástól eltérõ.

\n

2. Ábrázoljátok a 2000-es évtõl kezdve Európában a medián és az átlagos várható élettartam alakulását évenként.
    * *Hint:* a medián és az átlagos várható élettartam két különbözõ rétegben ábrázolható. 

\n

3. **A pie chart rossz** vagy csak [nagyon ritkán jó](https://medium.com/@KristinHenry/in-defense-of-pie-charts-and-why-you-shouldnt-use-them-df2e8ccb5f76). A következõ [blog bejegyzés](http://www.storytellingwithdata.com/blog/2014/08/design-with-audience-in-mind) egy pie chart makeover-rõl szól. Az új vizualizációt a cikkben Excelben készítik el, készítsetek hasonló ábrát R-ben. Az adatok letölthetõek a cikk végén található [linkrõl](https://www.dropbox.com/s/3ctpw1h1mje2fl0/USA%20Today%20Diversity%20Remake.xlsx?dl=0#)
    * *Hint1:* Az adatokat érdemes .txt vagy .csv formátumú fájlba másolni, majd `read.table()`, `read.csv()` vagy `read.delim()` függvénnyel beolvasni
    * *Hint2:* A megoszlásra vonatkozó adatokat *%* formázás nélkül olvassátok be, úgy, hogy a beolvasást követõen numerikus formátumú legyen. Figyeljetek a tizedes vesszõ/pont beállításra, ha ez nem stimmel, az R szöveges mezõként fogja felismerni az adatokat. Ehhez a `read.x()` függvényekben a `dec` paramétert kell helyesen beállítani.
    * *Hint3:* Az órán vertikális oszlopdigramokat láttunk, a feladatban viszont horizontális diagramot kell létrehozni. Ehhez meg kell fordítani a tengelyt a `coord_flip()` függvénnyel. A [hivatalos dokumentációban](http://ggplot2.tidyverse.org/reference/geom_bar.html) találtok ehhez leírást és példát is.
    * *Hint4:* Ha nagyon elakadtok a feladatmegoldás során, akkor a következõ [tutorial](http://rstudio-pubs-static.s3.amazonaws.com/4305_8df3611f69fa48c2ba6bbca9a8367895.html) egy egészen hasonló problémát old meg
